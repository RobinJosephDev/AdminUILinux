...
const Address: React.FC<AddressProps> = ({ company, setCompany }) => {
  ...
  const validateAndSetField = (field: keyof Company, value: string | boolean) => {
    ...
    if (['obsolete'].includes(field) && typeof value !== 'boolean') {
  ...
  const fields = [
    ...
    { label: 'Obsolete', key: 'obsolete', type: 'boolean' }
  ];
              ...
              {type === 'boolean' ? (
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
                  <input
                    type="checkbox"
                    id={key}
                    checked={!!company[key as keyof Company]}
