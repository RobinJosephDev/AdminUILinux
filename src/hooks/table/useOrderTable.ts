...
const useOrderTable = () => {
  ...
  const generateInvoice = () => {
    ...
    const customer = customers.find((c) => c.cust_name === order.customer);
    if (!customer) return;
    ...
    autoTable(doc, {
      ...
      didParseCell: function (data) {
        // Check if it's the "INVOICE No." row (first row) and first column (label)
        if (data.row.index === 0) {
    ...
    doc.text(customer.cust_name || 'N/A', boxX + 2, boxY + 12);
    doc.text(`Phone: ${customer.cust_contact_no || ''} Fax: ${customer.cust_ap_fax || ''}`, boxX + 2, boxY + 18);
    doc.text(`Email: ${customer.cust_email || ''}`, boxX + 2, boxY + 24);
    ...
    const tableBody = [
      [pickup.equipment, pickup.name || '', pickup.date || '-', delivery.pickup_po || '', delivery.date || '-', `$ ${order.final_price || '0.00'}`],
    ];
    ...
    doc.text(`Terms: -`, 14, afterTableY + 10);
    doc.text(`Due Date: -`, 14, afterTableY + 16);
    doc.text('Thank you for your business', 14, afterTableY + 24);

    // === RIGHT COLUMN ===
    const rightX = 120; // Adjust for alignment
    const subtotal = order.final_price || '0.00';

    doc.setFont('helvetica', 'bold');
    doc.setTextColor(0, 0, 0);

    doc.text(`Sub Total (CAD) = $ ${subtotal}`, rightX, afterTableY + 10);
    doc.text('Tax Exempt', rightX, afterTableY + 16);
    doc.text(`Total Amount (CAD) = $ ${subtotal}`, rightX, afterTableY + 24);
